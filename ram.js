...
export async function main(ns) {
	...
	let data = [];
	let servers = GetAllServers(ns).filter(s => ns.hasRootAccess(s) && !(s.includes(hServ))).sort((a, b) => ns.getServerMaxRam(b) - ns.getServerMaxRam(a));
	let details = [];
...
function GetProcessDetails(ns, server) {
	const categories = [
		...
		{ script: 'stanek-charge.js', header: 'Charge' }
	];
