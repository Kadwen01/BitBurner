	...
	*/
	...
	if (!corp.hasCorporation()) {
		...
		while (created == false) {
          ...
          ns.clearLog();
					ns.print("Not enough funds to create a corp yet. " + ns.formatNumber(ns.getPlayer().money, 2) + "/150b" );
	...
	function tendToWorkers() { // keep our employees happy
		...
		if (!corp.hasUnlock(unlockNames.OfficeAPI) && (corp.getCorporation().funds > corp.getUnlockCost(unlockNames.OfficeAPI))) {
			corp.purchaseUnlock(unlockNames.OfficeAPI);
		}
	...
	function expandWarehouse(div = "", level = 1) {
		...
		if (!corp.hasUnlock(unlockNames.WarehouseAPI) && (corp.getCorporation().funds > corp.getUnlockCost(unlockNames.WarehouseAPI))) {
			corp.purchaseUnlock(unlockNames.WarehouseAPI);
		}
