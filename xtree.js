...
export async function main(ns) {
	...
	const [hackingOnly = false] = ns.args;
	...
	for (let i = 0; i < servers.length; i++) {
		...
		if ((server.name.includes(pServ) || server.name.includes(hServ) || server.name === 'darkweb') && (ns.args[0] == 'npos')) {
			...
			let values = [
				{ color: thePill, text: ' ' + (hackingOnly ? '' : prefix) + server.name },
				//	{ color: 'white', text: server.sym ? ' ' + server.sym.padEnd(5) : ''.padStart(5) },
				{ color: maxRam > 0 ? freeRamColor : 'Grey', text: ' ' + freeRamString.padStart(7) + (maxRam == 0 ? ' ' : '/') + ramString.padStart(7) },
